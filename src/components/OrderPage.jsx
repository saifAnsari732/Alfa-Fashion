...
const OrderPage = () => {
  ...
  const handleWhatsAppOrder = (e) => {
    e.preventDefault();
    const text = `*New Order Request*%0A%0A*Name:* ${formData.name}%0A*Phone:* ${formData.phone}%0A*Category:* ${formData.category}%0A*Size/Age:* ${formData.size}%0A*Notes:* ${formData.notes}`;
    window.open(`https://wa.me/917985212241?text=${text}`, '_blank');
  };
          ...
          <div className="bg-white rounded-[2rem] shadow-xl p-8 sm:p-10 border border-slate-100">
            <form onSubmit={handleWhatsAppOrder} className="space-y-6">
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-4">
                ...
                <a href="tel:+917985212241" className="flex-1 bg-white border-2 border-slate-200 hover:border-[#2B231D] text-[#2B231D] py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
